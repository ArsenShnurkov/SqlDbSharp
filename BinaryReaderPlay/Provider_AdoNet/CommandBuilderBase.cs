namespace System.Data.SqlDbSharp
{
	...
	internal sealed class CommandBuilder: IDisposable
	{
		...
		internal IDbDataAdapter DataAdapter
		{
			...
			set
			{
				if (adapter != value)
				{
					...
					if (adapter != null)
					{
						if (adapter is SqlDbSharpDataAdapter)
						{
							sqlhandler = new SqlDbSharpRowUpdatingEventHandler(this.SqlRowUpdating);
							#if !POCKETPC
							((SqlDbSharpDataAdapter)adapter).RowUpdating += sqlhandler;
							#endif
		...
		private void SqlRowUpdating(object sender, SqlDbSharpRowUpdatingEventArgs ruevent)
		{
		...
		private void ApplyParameterInfo(SqlDbSharpParameter parameter, int pcount, DataRow row)
		{
#if POCKETPC
		...
		private IDbCommand BuildInsertCommand(DataTableMapping mappings, DataRow dataRow)
		{
			...
			for (int i = 0; i < rows; i++)
			{
				...
				if ((mappings != null) && (dataRow != null))
				{
					...
					if (value == null)
					{
						if (!(bool)row["IsReadOnly"] && !(command is SqlDbSharpCommand))
						{
							goto Close;
						}
						continue;
			...
			Close:
				...
				if (parameter is SqlDbSharpParameter)
				{
					this.ApplyParameterInfo((SqlDbSharpParameter) parameter, idx, row);
				}
		...
		private IDbCommand BuildUpdateCommand(DataTableMapping mappings, DataRow dataRow)
		{
			...
			for (int i = 0; i < rows; i++)
			{
				...
				if (((row != null) && (row["BaseColumnName"].ToString().Length != 0)) && !this.ExcludeFromUpdateSet(row))
				{
					...
					if (parameter is SqlDbSharpParameter)
					{
						this.ApplyParameterInfo((SqlDbSharpParameter) parameter, pcount, row);
					}
					if (!command.Parameters.Contains(parameter))
					{
						command.Parameters.Add(parameter);
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (dBSchemaRow != null && dBSchemaRow["BaseColumnName"].ToString().Length != 0 && IncludeForDeleteWhereClause(dBSchemaRow))
				{
					...
					if (!ispk || !Convert.IsDBNull(value))
					{
						...
						if (parameter is SqlDbSharpParameter)
						{
							ApplyParameterInfo((SqlDbSharpParameter)parameter, pcount, dBSchemaRow);
						}
						if (!ispk)
						{
							parameter.DbType = DbType.Int32;
		...
		private SqlDbSharpRowUpdatingEventHandler sqlhandler;
