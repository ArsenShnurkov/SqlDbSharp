namespace System.Data.SqlDbSharp.Linq
{
	...
	internal class SqlDbSharpProviderServices : DbProviderServices, ISqlDbSharpSchemaExtensions
	{
		internal static readonly SqlDbSharpProviderServices Instance = new SqlDbSharpProviderServices();
		...
		private DbCommand CreateCommand(DbProviderManifest manifest, DbCommandTree commandTree)
		{
			...
			SqlDbSharpCommand command = new SqlDbSharpCommand();
			try
			{
				List<DbParameter> parameters;
				CommandType commandType;

				command.CommandText = SqlGenerator.GenerateSql((SqlDbSharpProviderManifest)manifest, commandTree, out parameters, out commandType);
		...
		protected override string GetDbProviderManifestToken(DbConnection connection)
		{
			...
			if (connection is SqlDbSharpConnection)
				parseViaFramework = ((SqlDbSharpConnection)connection).ParseViaFramework;

			SortedList<string, string> opts = parseViaFramework ?
				SqlDbSharpConnection.ParseConnectionStringViaFramework(connection.ConnectionString, false) :
				SqlDbSharpConnection.ParseConnectionString(connection.ConnectionString);

			return SqlDbSharpConnection.FindKey(opts, "DateTimeFormat", "ISO8601");
		}

		protected override DbProviderManifest GetDbProviderManifest(string versionHint)
		{
			var res = new SqlDbSharpProviderManifest(versionHint);
			return res;
		}

		/// <summary>
		/// Creates a SqlDbSharpParameter given a name, type, and direction
		/// </summary>
		internal static SqlDbSharpParameter CreateSqlParameter(string name, TypeUsage type, ParameterMode mode, object value)
		{
			int? size;

			SqlDbSharpParameter result = new SqlDbSharpParameter(name, value);
		...
		#region ISqlDbSharpSchemaExtensions Members

		/// <summary>
		/// Creates temporary tables on the connection so schema information can be queried
		/// </summary>
		/// <remarks>
		/// There's a lot of work involved in getting schema information out of SqlDbSharp, but LINQ expects to
		...
		void ISqlDbSharpSchemaExtensions.BuildTempSchema(SqlDbSharpConnection cnn)
		{
			...
			using (SqlDbSharpCommand cmd = cnn.CreateCommand())
			{
		...
		private void DataTableToTable(SqlDbSharpConnection cnn, DataTable table, string dest)
		{
			StringBuilder sql = new StringBuilder();
			SqlDbSharpCommandBuilder builder = new SqlDbSharpCommandBuilder();
			...
			using (DataTable source = new DataTable())
			{
				...
				{
					DbType dbtypeName = SqlDbSharpConvert.TypeToDbType(dc.DataType);
					string typeName = SqlDbSharpConvert.DbTypeToTypeName(dbtypeName);
				...
				using (SqlDbSharpDataAdapter adp = new SqlDbSharpDataAdapter(cmd))
				{
