namespace System.Data.SqlDbSharp.Linq
{
	...
	internal static class DmlSqlGenerator
	{
		...
		private class ExpressionTranslator : DbExpressionVisitor
		{
			...
			internal SqlDbSharpParameter CreateParameter(object value, TypeUsage type)
			{
			...
			internal SqlDbSharpParameter CreateParameter(object value, DbType dbType)
			{
				string parameterName = string.Concat("@p", parameterNameCount.ToString(CultureInfo.InvariantCulture));
				parameterNameCount++;
				SqlDbSharpParameter parameter = new SqlDbSharpParameter(parameterName, value);
			...
			{
				SqlDbSharpParameter parameter = CreateParameter(expression.Value, expression.ResultType);
